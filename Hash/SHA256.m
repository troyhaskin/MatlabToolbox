function Hash = SHA256(Message)
    ...
    nZeros = max(mod(448 - (l+1),512),0);
    
    % Create the padded Message
    Pad =['1',repmat('0',1,nZeros),dec2bin(l,64)];
    M = [reshape(M',1,l),Pad];
    
    
    %   Split message into a vector of 32-bit words; 
    %   every 16 words comprises one 512-bit message block.
    nWords  = length(M)/32;
    nBlocks = nWords/16;
    iBlock  = 1:16;
    M = uint32(bin2dec(reshape(M,32,nWords)'));
    
    % Allocate expanded Message block array
    W = ones(64,1,'uint32');
    
    % Perform the compression for the first 16 iterations
    for k = 1:nBlocks
        % Assign intermediate hash values to registers
        a = H(1);   b = H(2);   c = H(3);   d = H(4);
        e = H(5);   f = H(6);   g = H(7);   h = H(8);
        
        % Assign current message block to expanded message block vector
        W(1:16) = M(iBlock + 16*(k-1));
        
        % Compression function: first 16 iterations
        for j = 1:16
        ...
        H = Mod32Add([[a;b;c;d;e;f;g;h],H]')';
    end
    
    % Convert intermediate hash value to the final output
    Hash = lower(reshape(dec2hex(H)',1,64));
